...
-- 2-2-1) gallery_items 공개/비공개 토글
alter table public.gallery_items
  add column if not exists is_active boolean not null default true;
create index if not exists gallery_items_active_idx
  on public.gallery_items (is_active) where is_active = true;
...
-- 2-4b) quick_menu_items (우측 플로팅 퀵메뉴, 최대 8개)
...
create index if not exists quick_menu_items_sort_idx on public.quick_menu_items (sort_order);
...
create policy "quick_menu public read"
  on public.quick_menu_items for select using (true);

drop policy if exists "admin can write quick_menu" on public.quick_menu_items;
create policy "admin can write quick_menu"
