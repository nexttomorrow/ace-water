  ...
  return (
    <div className="mx-auto max-w-[1100px] px-6 py-12">
      <div className="mb-1 text-[12px] text-neutral-500">
        <Link href="/admin/estimates" className="hover:underline">
          견적·도면 문의
        </Link>{' '}
        ›
      </div>
      <header className="mb-8 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Link
            href={`/admin/estimates/${inquiryId}/print`}
            target="_blank"
            className="inline-flex items-center gap-1.5 rounded bg-gradient-to-r from-blue-600 to-cyan-500 px-3.5 py-2 text-[13px] font-medium text-white shadow-sm transition hover:shadow-md"
          >
          ...
          <form action={updateAction} className="flex items-center gap-2">
            <select
              ...
              className="rounded border border-neutral-300 bg-white px-3 py-2 text-[13px]"
            >
            ...
            <button
              type="submit"
              className="rounded bg-neutral-900 px-3 py-2 text-[13px] font-medium text-white hover:bg-neutral-700"
            >
              상태 변경
            </button>
          </form>
          <form action={deleteAction}>
            <button
              type="submit"
              className="rounded border border-red-300 px-3 py-2 text-[13px] text-red-700 hover:bg-red-50"
            >
              삭제
            </button>
          </form>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-2">
          <DetailCard label="Request" title="요청사항">
            <div className="flex flex-wrap gap-1.5">
              {(it.request_types ?? []).map((v) => (
                <span
                  key={v}
                  className="rounded-full bg-blue-50 px-3 py-1 text-[12px] font-medium text-blue-800 ring-1 ring-blue-100"
                >
          ...
          {it.note && (
            <DetailCard label="Notes" title="참고사항">
              <p className="whitespace-pre-line text-[14px] leading-[1.85] text-neutral-700">
                {it.note}
              </p>
            </DetailCard>
          )}
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            ...
            <h3 className="mt-1.5 text-[15px] font-bold tracking-tight">첨부파일</h3>
            {attachmentUrl ? (
              <a
                href={attachmentUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-3 inline-flex w-full items-center justify-between gap-2 rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-3 text-[13px] font-medium text-neutral-800 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700"
              >
              ...
              <p className="mt-3 text-[13px] text-neutral-400">첨부 없음</p>
            )}
          </div>
          ...
          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            ...
            <h3 className="mt-1.5 text-[15px] font-bold tracking-tight">빠른 응대</h3>
  ...
  return (
    <section className="mb-4 rounded-xl border border-neutral-200 bg-white p-5">
      <header className="mb-3 border-b border-neutral-100 pb-2.5">
        ...
        <h3 className="mt-1 text-[15px] font-bold tracking-tight">{title}</h3>
      </header>
...
function Row({
  ...
  return (
    <div className="flex items-baseline gap-3">
      <span className="w-20 shrink-0 text-[11px] font-semibold uppercase tracking-[0.15em] text-neutral-400">
        {label}
      </span>
      <span className="flex-1 text-[14px] text-neutral-800">
  ...
  if (button) {
    return (
      <a
        ...
        className="flex items-center justify-center gap-2 rounded-full bg-neutral-900 px-4 py-2.5 text-[13px] font-medium text-white transition hover:bg-neutral-700"
      >
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
function EmailLink({
  ...
  if (button) {
    return (
      <a
        href={href}
        className="flex items-center justify-center gap-2 rounded-full border border-neutral-200 bg-white px-4 py-2.5 text-[13px] font-medium text-neutral-700 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700"
      >
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
