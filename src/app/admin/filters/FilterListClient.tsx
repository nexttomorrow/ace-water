        ...
        >
          ...
          {g.items.length === 0 ? (
            <p className="px-5 py-8 text-center text-[13px] text-neutral-400">
              등록된 필터가 없어요.{' '}
              <Link href="/admin/filters/new" className="text-blue-600 hover:underline">
                필터 추가
              </Link>
            </p>
          ) : (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                />
                <p className="truncate text-[14px] font-semibold">{f.label}</p>
...
function RowActions({ filter }: { filter: ProductFilter }) {
  ...
  return (
    <div className="flex shrink-0 items-center gap-1">
      ...
      <Link
        href={`/admin/filters/${filter.id}/edit`}
        className="rounded border border-neutral-300 px-2 py-1 text-[11px] hover:bg-neutral-100"
      >
        수정
      </Link>
