...
export async function createFilter(formData: FormData) {
  ...
  if (!fields.label) {
    redirect('/admin/filters/new?error=' + encodeURIComponent('필터 라벨을 입력해주세요'))
  }
  if (!fields.key) {
    redirect('/admin/filters/new?error=' + encodeURIComponent('필터 키를 입력해주세요'))
  }
  if (!AUTO_OPTION_KEYS.has(fields.key) && fields.options.length === 0) {
    redirect('/admin/filters/new?error=' + encodeURIComponent('옵션을 1개 이상 추가해주세요'))
  }

  const { error } = await supabase.from('product_filters').insert(fields)
  if (error) {
    redirect('/admin/filters/new?error=' + encodeURIComponent(error.message))
  }
...
export async function updateFilter(id: number, formData: FormData) {
  ...
  const fields = parseFilterFields(formData)
  if (!fields.label) {
    redirect(`/admin/filters/${id}/edit?error=` + encodeURIComponent('필터 라벨을 입력해주세요'))
  }
  if (!fields.key) {
    redirect(`/admin/filters/${id}/edit?error=` + encodeURIComponent('필터 키를 입력해주세요'))
  }
  if (!AUTO_OPTION_KEYS.has(fields.key) && fields.options.length === 0) {
    redirect(`/admin/filters/${id}/edit?error=` + encodeURIComponent('옵션을 1개 이상 추가해주세요'))
  }

  const { error } = await supabase.from('product_filters').update(fields).eq('id', id)
  if (error) {
    redirect(`/admin/filters/${id}/edit?error=` + encodeURIComponent(error.message))
  }
