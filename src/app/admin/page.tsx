  ...
  return (
    <div className="mx-auto max-w-[1440px] px-6 py-12">
      <div className="mb-8 flex items-end justify-between gap-4">
        ...
        <Link
          href="/admin/reports"
          className="inline-flex items-center gap-1.5 rounded-full bg-black px-5 py-2.5 text-[13px] font-bold text-white hover:bg-neutral-800"
        >
      ...
      <section className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
        {kpi.map((k) => (
          ...
          >
            <p className="text-[12px] font-medium text-neutral-500">{k.label}</p>
      ...
      <section className="mt-8 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="rounded-xl border border-neutral-200 bg-white p-5 lg:col-span-2">
          <header className="mb-4 flex items-baseline justify-between">
            <h2 className="text-[15px] font-bold text-neutral-900">최근 12개월 추이</h2>
            <span className="text-[11px] text-neutral-500">월별 등록 수</span>
          </header>
        ...
        <div className="rounded-xl border border-neutral-200 bg-white p-5">
          <header className="mb-4 flex items-baseline justify-between">
            <h2 className="text-[15px] font-bold text-neutral-900">견적문의 상태</h2>
            <span className="text-[11px] text-neutral-500">전체 누적</span>
          </header>
      ...
      <section className="mt-4 rounded-xl border border-neutral-200 bg-white p-5">
        <header className="mb-4 flex items-baseline justify-between">
          <h2 className="text-[15px] font-bold text-neutral-900">최근 6개월 견적문의</h2>
          <span className="text-[11px] text-neutral-500">월별 신규 접수</span>
        </header>
        <MonthlyBarChart
          labels={labels6}
          data={estimates6}
...
function DiffChip({ diff, pct }: { diff: number; pct: number | null }) {
  ...
  return (
    <div
      className={`mt-2 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ${tone}`}
    >
