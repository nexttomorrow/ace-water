    ...
    <>
      ...
      <div className="no-print sticky top-0 z-50 border-b border-neutral-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1100px] items-center justify-between gap-3 px-6 py-3">
          <div className="text-[0.875rem] text-neutral-500">
            <span className="font-semibold text-neutral-900">견적·도면 문의서</span>
            <span className="mx-2 text-neutral-300">·</span>#{id}
          </div>
          <div className="flex items-center gap-2">
            <a
              href={`/mng/estimates/${id}`}
              className="rounded-full border border-neutral-200 bg-white px-3.5 py-1.5 text-[0.75rem] font-medium text-neutral-700 transition hover:bg-neutral-50"
            >
              ← 상세로
            </a>
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-blue-600 to-cyan-500 px-4 py-1.5 text-[0.75rem] font-bold text-white shadow-sm transition hover:shadow-md"
            >
        ...
        <p className="mx-auto max-w-[1100px] px-6 pb-2 text-[0.75rem] text-neutral-500">
      ...
      <div className="min-h-screen bg-neutral-100 py-10 print:bg-white print:py-0">
        <article className="print-page mx-auto bg-white text-neutral-900">
          ...
          <header className="flex items-start justify-between border-b-2 border-neutral-900 pb-3">
            <div>
              <p className="text-[0.75rem] font-semibold uppercase tracking-[0.4em] text-neutral-500">
                {COMPANY.brand}
              </p>
              <h1 className="mt-1 text-[1.375rem] font-extrabold tracking-tight">
                견적·도면 문의서
              </h1>
            </div>
            <div className="text-right text-[0.75rem] leading-[1.65] text-neutral-600">
              <p>
          ...
          <Section title="요청사항">
            {requestLabels.length > 0 ? (
              ...
              <p className="text-[0.75rem] text-neutral-400">—</p>
            )}
          </Section>
              ...
              <tbody>
                ...
                {productColorRows.length > 0 && (
                  <tr className="border-b border-neutral-200 last:border-0">
                    <th className="w-[100px] bg-neutral-50 px-2.5 py-1.5 text-left align-top text-[0.75rem] font-semibold text-neutral-700">
                      색상
                    </th>
                    <td className="px-2.5 py-1.5 text-[0.75rem] text-neutral-900">
                      <div className="space-y-1">
                        {productColorRows.map((row, i) => (
                          <div key={i} className="flex flex-wrap items-center gap-x-2 gap-y-1">
                            <span className="font-mono text-[0.75rem] text-neutral-500">
                              {row.line}
                            </span>
                            ...
                            {row.colors.map((c, j) => (
                              <span
                                key={j}
                                className="inline-flex items-center gap-1 text-[0.75rem] text-neutral-700"
                                title={c.name}
                              >
                                <span
          ...
          {note && (
            <Section title="참고사항">
              <div className="min-h-[60px] whitespace-pre-line rounded border border-neutral-200 bg-neutral-50/50 p-3 text-[0.75rem] leading-[1.85]">
                {note}
              </div>
            </Section>
          )}
          ...
          {attachmentName && (
            <Section title="첨부">
              <p className="text-[0.75rem] text-neutral-700">📎 {attachmentName}</p>
            </Section>
          )}

          {/* 푸터 */}
          <footer className="mt-6 border-t-2 border-neutral-900 pt-3">
            <div className="grid grid-cols-2 gap-4 text-[0.75rem] leading-[1.7]">
              <div>
                <p className="text-[0.75rem] font-bold tracking-tight text-neutral-900">
            ...
            <p className="mt-2 text-center text-[0.75rem] text-neutral-400">
...
function Section({
  ...
  return (
    <section className="mt-3.5">
      <h2 className="mb-1.5 border-b border-neutral-900 pb-1 text-[0.75rem] font-bold tracking-tight">
        {title}
      </h2>
      {children}
    </section>
...
function Row({
  ...
  return (
    <tr className="border-b border-neutral-200 last:border-0">
      <th className="w-[100px] bg-neutral-50 px-2.5 py-1.5 text-left text-[0.75rem] font-semibold text-neutral-700">
        {label}
      </th>
      <td className={`px-2.5 py-1.5 text-[0.75rem] text-neutral-900 ${mono ? 'font-mono' : ''}`}>
        {value}
      </td>
    </tr>
  )
