...
import { adminBulkDeleteNotices } from '../actions'
...
export default function NoticeListClient({ notices }: { notices: NoticeRow[] }) {
  const ids = notices.map((p) => p.id)

  return (
    <SelectableTable
      ids={ids}
      onBulkDelete={async (ids) => {
        await adminBulkDeleteNotices(ids)
      }}
      deleteConfirm="선택한 공지사항을 삭제하시겠습니까?"
    >
          ...
          <div className="overflow-x-auto">
            <table className="w-full table-fixed text-sm">
              ...
              <tbody className="divide-y divide-neutral-200">
                {notices.map((p) => {
                  const handleDelete = deleteNotice.bind(null, p.id)
                  return (
